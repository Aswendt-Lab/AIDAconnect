function analyzeLostConnections(inputFMRI, graphCell, selectedRegion, day1, day2)

%% analyzeLostConnections
% This function displays connections of a certain region that are not
% present in the second time point but do have a connection weight in the
% first time point. You can specify a weighting threshold to neglect
% lost connections with small weights in line 17. In case of rsfMRI, 
...
%% Example
% analyzeLostConnections(inputFMRI, graphCell, "L SSp-ll", "Baseline", "P7")
...
for group_id = 1:size(inputFMRI.groups,2)
    ...
    % expressions
    nonZeroLogical = allConnectionsI(zeroIndices)~=0;
    nonZeroIndices = zeroIndices(nonZeroLogical);
    nonZeroIndicesWeight = allConnectionsI(nonZeroIndices);   
    ...
    greaterThanThresholdWeights = round(allConnectionsI(greaterThanThresholdIndices),3);
    ...
    disp('Lost connections of '+string(selectedRegion)+' in group '+inputFMRI.groups(group_id)+' from '+day1+' to '+day2+':');
