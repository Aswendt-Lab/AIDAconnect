function analyzeMostConnected(inputDTI, graphCell, RegionPF, day, out_path)
...
% You can save the table as analyzeMostConnected.csv if you specify an output path 
...
groupTable.Properties.VariableNames = {...
    strcat(char(inputDTI.groups(1)),' Left'),'W1',...
    strcat(char(inputDTI.groups(1)),' Right'),'W2',...
    strcat(char(inputDTI.groups(2)),' Left'),'W3',...
    strcat(char(inputDTI.groups(2)),' Right'),'W4'};
display(groupTable);

disp('Figure 1 represents (Group 1): '+string(inputDTI.groups(1)));
disp('Figure 2 represents (Group 2): '+string(inputDTI.groups(2)));
...
for i = 1:2
    ...
    yd = get(p_R, 'YData');
    zd = get(p_R, 'ZData');
    text(xd+10, yd, zd, nl, 'FontSize', 10);
end

% Figure Group 2
figure('Name', inputDTI.groups(2));
set(gcf,'color','w');
for i = 1:2
    subplot(1,2,i)
    if displayOptions == 1
