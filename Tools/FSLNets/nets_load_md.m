...
%
% ts = nets_load(indir,prefix,tr,varnorm);
% ts = nets_load(indir,prefix,tr,varnorm,Nruns);
%
...
function [ts] = nets_load_md(indir,prefix,tr,varnorm,varargin)
...
%d=dir('*.txt');                % michaeld
d=dir(strcat(fullfile(indir,prefix),'*.txt'));  % michaeld
Nsubjects=size(d,1);
TS=[];
for i=1:Nsubjects
  disp(d(i).name);
  grotALL=load(fullfile(d(i).folder,d(i).name));  gn=size(grotALL,1);
