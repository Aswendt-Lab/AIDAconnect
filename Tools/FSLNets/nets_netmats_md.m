...
function [netmats] = nets_netmats_md(ts,do_rtoz,do_rtoz_corr,method,varargin)
...
for s=1:ts.Nsubjects
  ...
  switch lower(method)
    ...
    case {'icov','partial'}
      grot=cov(grot);
      if nargin==4     % simple partial correlation
        grot=-inv(grot);
...
if do_rtoz==1 && MethodType>0
  ...
  if do_rtoz_corr==1
    ...
    netmats=0.5*log((1+netmats)./(1-netmats));
  end
end
