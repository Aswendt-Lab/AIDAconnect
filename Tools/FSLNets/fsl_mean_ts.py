...
def save_csv(sFilename, data):
    with open(sFilename, 'wb') as fid:
        writer = csv.writer(fid, delimiter=' ', dialect='excel')
        writer.writerows(data)

def save_data(sFilename, data):
    with open(sFilename, 'wb') as fid:
        for sLine in data:
            print('  '.join(sLine), end='  ' + chr(10), file=fid)
...
if __name__ == '__main__':
    ...
    # output data
    sPathOut = args.out_text if args.out_text != None else os.path.abspath(os.path.join(sPathData, os.pardir, 'MasksTCs.' + os.path.split(sPathData)[1]))
    if sPathOut.endswith('.nii.gz'):
        sPathOut = sPathOut[:-7]
    elif sPathOut.endswith('.nii'):
        sPathOut = sPathOut[:-4]
    ...
    if len(data_shape) != 4:
        sys.exit("Error: data %s is not 4D shape." % (str(data_shape),))
    ...
    if len(mask_shape) != 4:
        sys.exit("Error: mask %s is not 4D shape." % (str(mask_shape),))
    ...
    np.savetxt(sPathOut, np.transpose(m) , fmt='%.4f',delimiter=' ')
    #print(sPathOut)
    #save_csv(sPathOut, s)
